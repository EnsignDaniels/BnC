...
SOURCE_PCGLNS_PATH ='PCGLNS/PCGLNS_PCGTSP/'
...
def parseResult(task_name):
	with open(f'{HEURISTIC_PATH}{task_name}{HEURISTIC_SOLUTION_SUFFIX}') as f:
		ln = f.readline().strip()
		while ln:
...
def runHeuristic(task_name, source_path = SOURCE_PCGLNS_PATH):
	result_filename = f'{HEURISTIC_PATH}{task_name}{HEURISTIC_SOLUTION_SUFFIX}'
	if not os.path.isfile(result_filename):
		sp.run([f'{PCGLNS_ENGINE}', f'{SCRIPT_PATH_NAME}',f'{source_path}{task_name}{SOURCE_PCGLNS_SUFFIX}', f'-output={result_filename}'])
