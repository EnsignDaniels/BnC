...
############################
	test_tour = [1, 33, 30, 26, 28, 27, 18, 41, 40, 47, 35, 31, 4, 14, 12, 7, 50, 53, 34, 32, 3, 2, 42, 44, 6, 52, 51, 49, 8, 15, 46, 45, 38, 36, 39, 5, 23, 43, 19, 9, 17, 21, 22, 25, 24, 48, 29, 10, 37, 11, 13, 16, 20, 1]
	print(f'Test_tour: {test_tour}')
	
	test_u = {(p,q): 0 for (p,q) in permutations(clusters,2)}
	test_y = {(p,q): 0 for (p,q) in permutations(clusters,2)}
	
	c_pred = 1
	idx_cur = 1
	for c_cur in test_tour[1:]:
		test_u[(c_pred,c_cur)] = 1
		for c2 in test_tour[idx_cur:-1]:
			test_y[c_pred,c2] = 1
		c_pred = c_cur
		idx_cur += 1
	

	model._test_u = test_u
	model._test_y = test_y
	print('Test ft53.4 is ready')
